...
client = brawlstats.Client("окак") # type: ignore
...
COINS_PER_MAX_BRAWLER = 17765  # Базовая: 11 лвл + 1 гаджет(1000) + 1 пассивка(2000) + 2 снаряжения(2000) + гиперзаряд(5000)
COINS_PER_MAX_BRAWLER_FULL = 24765  # Полная: +  1 гаджет(1000) + 1 пассивка(2000) + 3 снаряженй окак макс нахуя тьы это читаешь☠️☠️


TOTAL_COINS_FULL_ACCOUNT = 95 * COINS_PER_MAX_BRAWLER_FULL
TOTAL_POWER_POINTS_FULL_ACCOUNT = 336600  # × 3270 очков силы
...
for brawler in brawlers:
    ...
    # БАЗОВАЯ прокачка: 11 лвл + 1 гаджет + 1 пассивка + 2 снаряжения + гиперзаряд
    ...
    coins_spent_base += gears_count_base * 1000  # 2 снаряжения
    
    # Если перс 11 лвла, добавляем стоимость гиперзаряда (5000)
    if brawler.power == 11:
    ...
    # Если перс 11 уровня, добавляем стоимость гипера (5000)
    if brawler.power == 11:
...
# Расчет прогресса павер поинтов
...
# окак типа линия ☠️☠️ 3 очки силы
...
print(f"\n💡 ДЛЯ СПРАВКИ:")
print(f"Полная прокачка всего аккаунта ({len(brawlers)}/95 бойцов):")
